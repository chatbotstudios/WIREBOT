...
public:
    ...
    void begin(const char *provider_str, const char *api_key, const char *model, const char *base_url = nullptr);
...
private:
    ...
    int buildGeminiRequest(char *buf, int buf_len, const LlmMessage *messages, int count, const char *tools_json);
    bool parseGeminiResponse(const char *body, int body_len, LlmResult *result);

    /* Parse tool_calls array from response body (OpenAI-compatible) */
