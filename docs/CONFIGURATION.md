...
| WiFi SSID | Yes | - |
| WiFi Password | Yes | - |
| LLM Provider | No | `openrouter` |
| API Key | Yes* | - |
| Model | No | `google/gemini-2.5-flash` |
...
- `openrouter` (Default): Connects to OpenRouter. Use an OpenRouter API key.
- `deepseek`: Connects directly to DeepSeek (`api.deepseek.com`). Use a DeepSeek API key.
- `gemini`: Connects directly to Google's Gemini API (`generativelanguage.googleapis.com`). Use a Google AI Studio API key. Native support handles the unique request format.
- `openai_compatible`: For local servers (like Ollama or vLLM) or other OpenAI-compatible endpoints. Requires `api_base_url` to be set.
...
For developers, WireClaw supports generating `data/config.json` at build time from a `.env` file. This avoids manual configuration via the web portal or serial console.

1. Copy `.env.example` to `.env`.
2. Fill in your WiFi, LLM API key, provider, and other settings.
3. When you run `pio run -t uploadfs` (or any build task), a python script (`scripts/generate_config.py`) automatically merges your `.env` variables with defaults and generates `data/config.json`.
4. Flash the filesystem and the device will boot with your configuration ready to go.
