...
static bool handleClient(WiFiClient &client) {
    ...
    if (isPost && contentLength > 0) {
        ...
        int toRead = contentLength < (int)2048 - 1
                     ? contentLength : (int)2048 - 1;
...
void runSetupPortal() {
    ...
    while (millis() - startTime < PORTAL_TIMEOUT_MS) {
        ...
        extern void boardUpdateUI(const char *device_name, bool wifi_connected, const char *ip_address);
        static unsigned long last_ui_update = 0;
        if (millis() - last_ui_update > 1000) {
            last_ui_update = millis();
            boardUpdateUI("SETUP MODE", false, "AP: WireClaw-Setup");
        }
#endif
