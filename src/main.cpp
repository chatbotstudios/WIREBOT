...
char cfg_discord_token[128];
char cfg_discord_user_id[32];
char cfg_discord_channel_id[32];
...
static void configDefaults() {
  ...
  cfg_discord_token[0] = '\0';
  cfg_discord_user_id[0] = '\0';
  cfg_discord_channel_id[0] = '\0';
  strncpy(cfg_timezone, "UTC0", sizeof(cfg_timezone));
  strncpy(cfg_system_prompt,
          "You are WireClaw, a helpful AI assistant running on an ESP32 "
          "microcontroller. "
 ...
 */
static bool jsonGetString(const char *json, const char *key, char *dst,
                          int dst_len) {
  char pattern[64];
...
static bool loadConfig() {
  ...
  if (len > 0) {
    ...
    if (!jsonGetString(json_buf, "llm_provider", cfg_llm_provider,
                       sizeof(cfg_llm_provider))) {
      strncpy(cfg_llm_provider, "openrouter", sizeof(cfg_llm_provider));
    }
    jsonGetString(json_buf, "model", cfg_model, sizeof(cfg_model));
    jsonGetString(json_buf, "device_name", cfg_device_name,
                  sizeof(cfg_device_name));
    jsonGetString(json_buf, "api_base_url", cfg_api_base_url,
                  sizeof(cfg_api_base_url));
    ...
    jsonGetString(json_buf, "telegram_token", cfg_telegram_token,
                  sizeof(cfg_telegram_token));
    jsonGetString(json_buf, "telegram_chat_id", cfg_telegram_chat_id,
                  sizeof(cfg_telegram_chat_id));
    jsonGetString(json_buf, "discord_token", cfg_discord_token,
                  sizeof(cfg_discord_token));
    jsonGetString(json_buf, "discord_user_id", cfg_discord_user_id,
                  sizeof(cfg_discord_user_id));
    jsonGetString(json_buf, "discord_channel_id", cfg_discord_channel_id,
                  sizeof(cfg_discord_channel_id));
...
bool g_debug = false;
bool g_led_user =
    false; /* true when LED was set by a tool - don't overwrite with status */
#if !defined(CONFIG_IDF_TARGET_ESP32)
...
static void historySave() {
  ...
  if (g_debug)
    Serial.printf("History: saved %d turns\n", historyCount);
}
...
void initTempSensor() {
  ...
  if (err != ESP_OK) {
    Serial.printf("Temp sensor enable failed: %d\n", err);
  }
}
#endif
...
const char *chatWithLLM(const char *userMessage) {
  ...
  g_led_user =
      false; /* Reset - status LEDs allowed until a tool sets the LED */
  ledBlue(); /* Thinking... */
  ...
  for (int iter = 0; iter < MAX_AGENT_ITERATIONS; iter++) {
    ...
    if (!g_led_user)
      ledPurple(); /* Show we're in a tool loop */
  }
...
static void onNatsChat(nats_client_t *client, const nats_msg_t *msg,
  ...
  size_t len =
      msg->data_len < sizeof(chatBuf) - 1 ? msg->data_len : sizeof(chatBuf) - 1;
...
static bool handleCommand(const char *cmd, char *buf, int buf_len) {
  ...
  if (strcmp(cmd, "status") == 0) {
    snprintf(buf, buf_len,
             ...
             ESP.getHeapSize(), historyCount, cfg_model, g_debug ? "ON" : "OFF",
             g_nats_enabled ? (g_nats_connected ? "connected" : "disconnected")
                            : "disabled",
             g_telegram_enabled ? "enabled" : "disabled", millis() / 1000);
    return true;
  }
  ...
  if (strcmp(cmd, "devices") == 0) {
    ...
    for (int i = 0; i < MAX_DEVICES && w < buf_len - 80; i++) {
      ...
      if (d->kind == DEV_SENSOR_SERIAL_TEXT) {
        ...
        w += snprintf(buf + w, buf_len - w, "%s [serial_text] %ubaud = %.1f %s",
                      d->name, (unsigned)d->baud, val, d->unit);
      } else if (d->kind == DEV_SENSOR_NATS_VALUE) {
        float val = deviceReadSensor(d);
        w += snprintf(buf + w, buf_len - w, "%s [nats_value] %s = %.1f %s",
                      d->name, d->nats_subject, val, d->unit);
      } else if (deviceIsSensor(d->kind)) {
        float val = deviceReadSensor(d);
        w += snprintf(buf + w, buf_len - w, "%s [%s] pin=%d = %.1f %s", d->name,
                      deviceKindName(d->kind), d->pin, val, d->unit);
      } else {
        w += snprintf(buf + w, buf_len - w, "%s [%s] pin=%d%s", d->name,
                      deviceKindName(d->kind), d->pin,
                      d->inverted ? " (inverted)" : "");
      }
    }
    if (w == 0)
      snprintf(buf, buf_len, "No devices");
    return true;
  }
  if (strcmp(cmd, "rules") == 0) {
    ...
    for (int i = 0; i < MAX_RULES && w < buf_len - 120; i++) {
      ...
      w += snprintf(buf + w, buf_len - w,
                    ...
                    r->last_reading, r->fired ? "FIRED" : "idle",
      ...
      if (r->on_action == ACT_LED_SET) {
        ...
        w += snprintf(buf + w, buf_len - w, "(%d,%d,%d)", (v >> 16) & 0xFF,
                      (v >> 8) & 0xFF, v & 0xFF);
        ...
        w += snprintf(buf + w, buf_len - w, " pin=%d val=%d", r->on_pin,
                      (int)r->on_value);
      }
      /* OFF action */
      if (r->has_off_action) {
        ...
        if (r->off_action == ACT_LED_SET) {
          ...
          w += snprintf(buf + w, buf_len - w, " \"%s\"", r->off_nats_pay);
        } else if (r->off_action == ACT_ACTUATOR) {
          w += snprintf(buf + w, buf_len - w, " %s", r->off_actuator);
        } else if (r->off_action == ACT_GPIO_WRITE) {
          w += snprintf(buf + w, buf_len - w, " pin=%d val=%d", r->off_pin,
                        (int)r->off_value);
        }
      }
      /* Chain links */
    ...
    if (w == 0)
      snprintf(buf, buf_len, "No rules");
    return true;
  }
  if (strcmp(cmd, "memory") == 0) {
    int len = readFile("/memory.txt", buf, buf_len);
    if (len <= 0)
      snprintf(buf, buf_len, "(no memory file)");
    return true;
  }
  if (strcmp(cmd, "time") == 0) {
    struct tm timeinfo;
    if (getLocalTime(&timeinfo, 0)) {
      snprintf(buf, buf_len, "%04d-%02d-%02d %02d:%02d:%02d (TZ=%s)",
  ...
  if (strcmp(cmd, "history") == 0) {
    ...
    for (int i = 0; i < historyCount && w < buf_len - 80; i++) {
      ...
      w += snprintf(buf + w, buf_len - w, "[%d] %.40s%s\n  -> %.60s%s\n", i + 1,
                    history[i].user, strlen(history[i].user) > 40 ? "..." : "",
                    history[i].assistant, alen > 60 ? "..." : "");
    }
    return true;
  }
...
static void onNatsCmd(nats_client_t *client, const nats_msg_t *msg,
  ...
  size_t len =
      msg->data_len < sizeof(cmdBuf) - 1 ? msg->data_len : sizeof(cmdBuf) - 1;
...
static void onNatsToolExec(nats_client_t *client, const nats_msg_t *msg,
  ...
  if (strcmp(toolName, "remote_chat") == 0) {
    ...
    if (msg->reply_len > 0)
      nats_msg_respond_str(client, msg,
                           "{\"ok\":false,\"error\":\"remote_chat not "
                           "available via tool_exec\"}");
    return;
  }
  ...
  if (strcmp(toolName, "file_write") == 0) {
    ...
    if (jsonGetString(toolCallJsonBuf, "path", pathBuf, sizeof(pathBuf)) &&
      ...
      if (msg->reply_len > 0)
        nats_msg_respond_str(client, msg,
                             "{\"ok\":false,\"error\":\"cannot write to "
                             "/memory.txt via tool_exec\"}");
      return;
    }
  }
  ...
  if (g_nats_connected) {
    ...
    snprintf(evtBuf, sizeof(evtBuf),
             "{\"event\":\"tool_exec\",\"tool\":\"%s\",\"ok\":%s}", toolName,
             ok ? "true" : "false");
...
static void onNatsValue(nats_client_t *client, const nats_msg_t *msg,
  ...
  if (!dev || !dev->used)
    return;
  parseNatsPayload(msg->data, msg->data_len, &dev->nats_value, dev->nats_msg,
                   sizeof(dev->nats_msg));
  if (g_debug)
    Serial.printf("[NATS] %s = %.1f (msg='%s')\n", dev->name, dev->nats_value,
                  dev->nats_msg);
}
...
void natsSubscribeDeviceSensors() {
  ...
  for (int i = 0; i < MAX_DEVICES; i++) {
    if (!devs[i].used)
      continue;
    if (devs[i].kind != DEV_SENSOR_NATS_VALUE)
      continue;
    if (devs[i].nats_subject[0] == '\0')
      continue;
    if (devs[i].nats_sid != 0)
      continue; /* already subscribed */
    uint16_t sid = 0;
    nats_err_t err =
        natsClient.subscribe(devs[i].nats_subject, onNatsValue, &devs[i], &sid);
    if (err == NATS_OK) {
      devs[i].nats_sid = sid;
      Serial.printf("[NATS] Subscribed '%s' -> %s (sid=%d)\n", devs[i].name,
                    devs[i].nats_subject, sid);
    } else {
...
static void buildNatsSubjects() {
  snprintf(natsSubjectChat, sizeof(natsSubjectChat), "%s.chat",
           cfg_device_name);
  snprintf(natsSubjectCmd, sizeof(natsSubjectCmd), "%s.cmd", cfg_device_name);
  snprintf(natsSubjectEvents, sizeof(natsSubjectEvents), "%s.events",
           cfg_device_name);
  snprintf(natsSubjectToolExec, sizeof(natsSubjectToolExec), "%s.tool_exec",
           cfg_device_name);
  snprintf(natsSubjectCapabilities, sizeof(natsSubjectCapabilities),
           "%s.capabilities", cfg_device_name);
  snprintf(natsSubjectHal, sizeof(natsSubjectHal), "%s.hal.>", cfg_device_name);
}
...
static bool connectNats() {
  ...
  err = natsClient.subscribe(natsSubjectCapabilities, onNatsCapabilities,
                             nullptr);
  if (err != NATS_OK) {
    Serial.printf("NATS: subscribe %s failed: %s\n", natsSubjectCapabilities,
                  nats_err_str(err));
  }
  ...
  snprintf(onlineMsg, sizeof(onlineMsg),
           ...
           "\"hal\":\"%s\"}",
           cfg_device_name, WIRECLAW_VERSION, WiFi.localIP().toString().c_str(),
           natsSubjectToolExec, natsSubjectCapabilities, natsSubjectHal);
  natsClient.publish(natsSubjectEvents, onlineMsg);

  Serial.printf("NATS: subscribed to %s, %s, %s, %s, %s\n", natsSubjectChat,
                natsSubjectCmd, natsSubjectToolExec, natsSubjectCapabilities,
                natsSubjectHal);
...
static int tgApiCall(const char *method, const char *body, int body_len,
  ...
  if (!tgClient.connect(TG_HOST, TG_PORT)) {
    if (g_debug)
      Serial.printf("[TG] Connect failed\n");
    return -1;
  }
  ...
  if (content_length > 0) {
    ...
    while (total < buf_len - 1) {
      ...
      if (avail > 0) {
        int rd =
            tgClient.readBytes(buf + total, min(avail, buf_len - 1 - total));
        total += rd;
        last_data = millis();
...
static void telegramTick() {
  ...
  case TG_IDLE: {
    ...
    tgWaitStart = now;
    if (g_debug)
      Serial.printf("[TG] Long poll started\n");
    return;
  }
  ...
  case TG_WAITING: {
    if (!tgClient.available()) {
      ...
      if (!tgClient.connected()) {
        if (g_debug)
          Serial.printf("[TG] Disconnected during wait\n");
        tgClient.stop();
        tgState = TG_IDLE;
        tgLastPoll = now;
        return;
      ...
      if (now - tgWaitStart > TG_WAIT_TIMEOUT) {
        if (g_debug)
          Serial.printf("[TG] Long poll timeout\n");
        tgClient.stop();
        tgState = TG_IDLE;
        tgLastPoll = now;
        return;
    ...
    int total = 0;
    if (content_length > 0) {
      int to_read = content_length < (int)sizeof(resp) - 1
                        ? content_length
                        : (int)sizeof(resp) - 1;
      ...
      while (total < (int)sizeof(resp) - 1) {
        int avail = tgClient.available();
        if (avail > 0) {
          int rd = tgClient.readBytes(
              resp + total, min(avail, (int)sizeof(resp) - 1 - total));
          total += rd;
          last_data = millis();
        } else if (!tgClient.connected()) {
          break;
    ...
    if (g_debug)
      Serial.printf("[TG] update_id=%d (last=%d)\n", update_id, tgLastUpdateId);
    if (update_id <= tgLastUpdateId)
      return;
    ...
    if (!chat_id_str) {
      if (g_debug)
        Serial.printf("[TG] no chat field\n");
      return;
    }
    const char *id_str = strstr(chat_id_str, "\"id\"");
    if (!id_str) {
      if (g_debug)
        Serial.printf("[TG] no id in chat\n");
      return;
    }
    p = id_str + 4;
    while (*p == ':' || *p == ' ')
    ...
    if (g_debug)
      Serial.printf("[TG] chat_id=%s (allowed=%s)\n", incoming_chat_id,
    ...
    if (!text_key) {
      if (g_debug)
        Serial.printf("[TG] no text field\n");
      return;
    }
    p = text_key + 6;
    while (*p == ':' || *p == ' ')
    ...
    if (*p != '"') {
      if (g_debug)
        Serial.printf("[TG] text not a string\n");
      return;
    }
    p++;
...
static int discordApiCall(const char *method, const char *endpoint,
                          const char *body, int body_len, char *buf,
                          int buf_len) {
  discordClient.stop();
  if (!discordClient.connect(DISCORD_API_HOST, DISCORD_API_PORT)) {
    if (g_debug)
      Serial.printf("[Discord] Connect failed\n");
    return -1;
  }
  ...
  if (content_length > 0) {
    ...
    while (total < buf_len - 1) {
      ...
      if (avail > 0) {
        int rd = discordClient.readBytes(buf + total,
                                         min(avail, buf_len - 1 - total));
        total += rd;
...
void setup() {
  ...
  /* Init LLM client */
  llm.begin(cfg_llm_provider, cfg_api_key, cfg_model, cfg_api_base_url);
  ...
  if (cfg_telegram_token[0] != '\0' && cfg_telegram_chat_id[0] != '\0') {
    ...
    snprintf(startMsg, sizeof(startMsg),
             ...
             WIRECLAW_VERSION, WiFi.localIP().toString().c_str(),
             cfg_device_name);
    tgSendMessage(startMsg);
  } else {
    Serial.printf(
        "Telegram: disabled (no telegram_token/telegram_chat_id in config)\n");
  }
...
void loop() {
  ...
  if (g_nats_enabled) {
    if (natsClient.connected()) {
      ...
      if (err != NATS_OK && err != NATS_ERR_WOULD_BLOCK) {
        if (g_debug)
          Serial.printf("NATS: process error: %s\n", nats_err_str(err));
      }
    } else {
  ...
  if (millis() - last_ui_update > 1000) {
    ...
    boardUpdateUI(cfg_device_name, WiFi.status() == WL_CONNECTED, WiFi.localIP().toString().c_str());
  }
#endif
